...
flags.DEFINE_float("bc_learning_rate", 0.01,
                   "Learning rate for avg-policy behavorial cloning network.")
...
# TODO does this place non-zero probability on illegal actions?
def _make_bc_network(environment_spec):
...
def main(_):
  ...
  for i in range(environment.num_players):
    agents.append(
        nfsp.NFSP(
            ...
            bc_network=_make_bc_network(environment_spec),
  ...
  # TODO uncomment to turn one agent into a random actor.
  ...
  loop.run(num_episodes=FLAGS.num_train_episodes)
