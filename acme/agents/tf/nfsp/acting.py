...
class NFSPActor(core.Actor):
  ...
  @tf.function
  def _bc_policy(self, observation: types.NestedTensor) -> types.NestedTensor:
    # Add a dummy batch dimension and as a side effect convert numpy to TF.
    batched_observation = tf2_utils.add_batch_dim(observation)

    # Compute the policy, conditioned on the observation.
    logits = self._bc_network(batched_observation)
  ...
  def select_action(self, observation: types.NestedArray) -> types.NestedArray:
    ...
    if self._mode == MODE.best_response:
      ...
      action = self._bc_policy(observation)
    else:
  ...
  # TODO check this
  # TODO Reservoir sampling for bc_adder
  ...
  def observe_first(self, timestep: dm_env.TimeStep):
    ...
    if self._mode == MODE.best_response and self._bc_adder:
      self._bc_adder.add_first(timestep)
  ...
  def observe(self, action: types.NestedArray, next_timestep: dm_env.TimeStep):
    ...
    if self._mode == MODE.best_response and self._bc_adder:
      self._bc_adder.add(action, next_timestep)
