...
"""An OpenSpiel multi-agent/environment training loop for NFSP."""
...
class NFSPPolicies(policy.Policy):
  ...
  def action_probabilities(self, state, player_id=None):
    ...
    legals[legal_actions] = 1.0  # TODO use 1. instead? pylint
    player_observation = OLT(observation=np.asarray(state.information_state_tensor(cur_player), dtype=np.float32),
                             legal_actions=legals,
                             terminal=np.asarray([float(state.is_terminal())], dtype=np.float32))  # TODO best practice?

    if self._mode == "AVG":
      policy = self._actors[cur_player]._actor._get_avg_policy(player_observation).numpy()
      policy = np.squeeze(policy)
      
    elif self._mode == "BR":
      batched_observation = tf2_utils.add_batch_dim(player_observation)
      action_values = np.squeeze(self._actors[cur_player]._learner._rl_learner._network(batched_observation))  # TODO hacky?
      policy = np.zeros(self.game.num_distinct_actions(), dtype=np.float32)
      policy[np.argmax(action_values)] = 1
    else:
      raise ValueError("Invalid mode")

    prob_dict = {action: policy[action] for action in legal_actions}
    return prob_dict



class NFSPEnvironmentLoop(core.Worker):
  # TODO
  ...
  def __init__(
      ...
      label: str = 'nfsp_environment_loop',
      eval_every: int = -1,  # TODO need to configure
      evaluation_metric: str = 'exploitability', # TODO need to configure
  ):
...
#        print("[{}] AVG Value against Random {}".format(episode_count, policy_value))
        ...
        for agent in self._actors:
            print(agent._bc_buffer_client.server_info())
